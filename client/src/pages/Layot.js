...
function Layot() {
  const userData = useSelector((state) => state.database.value.user);
    ...
    <>
      <section className="w-full h-[100vh] relative flex flex-col items-center">
        <nav className="w-full h-[70px] bg-[#0000003f] rounded-b-[10px] absolute top-0 z-20 flex justify-between px-[80px] ">
          ...
          <div dir="rtl" className="w-[50%] h-[70px] flex items-center ">
            <img
              src={userData.imgurl}
              className="rounded-[50%] w-[60px] h-[60px] object-cover"
            />
            <div className="mr-[15px] flex w-[90px] text-[#000000b6] text-[12px] font-semibold">
              <p>{userData.name}</p>
              <p className="mr-1">{userData.username}</p>
            </div>
          </div>
        </nav>
        <img src={blogyimg} className="w-full h-[100vh] blur-[8px] " />
        <div className=" absolute w-full h-[100vh] top-0 right-0 left-0 bottom-0 bg-[#0000003d] "></div>
        <section className="w-[100%] h-[100vh] bg-[#0000002c] m-auto absolute top-0 px-[80px] flex items-end justify-center  "></section>
      </section>
      <Outlet />
    </>
  );
